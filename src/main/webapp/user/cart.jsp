...
<div class="container">
    ...
    <table>
        ...
        <tr>
            <td><%= p.getProName() %></td>
            <td><input type ="date" name ="orderArrivedDay" ></td>
            ...
            <td><%= p.getProId() %></td>
            <td>
                <form method="post" action="<%= request.getContextPath() %>/deleteCartItem">
                    <input type="hidden" name="productId" value="<%= p.getProId() %>">
                    <button type="submit" class="action-button">削除</button>
                </form>
            </td>
            <td>
                <form method="post" action="<%= request.getContextPath() %>/updateCartItem">
<!--                    <input type="hidden" name="oderId[]" value="<%= item.getOrderId()%>">-->
<!--                    <input type ="hidden" name ="orderArrivedDay[]"  value="<%= item.getOrderArrivedDay() %>">           -->
                    <input type="number" name="quantity" value="" min="1" style="width: 60px;">
    ...
    <div class="button-group">
        ...
        <form action="<%= request.getContextPath() %>/updateCartItem" method="post">
