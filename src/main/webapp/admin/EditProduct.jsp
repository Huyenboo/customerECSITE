...
<body>
  <div class="container">
    ...
    <form action="<%=request.getContextPath()%>/EditProductServlet" method="post">
      ...
      <div class="field"><label>商品ID:</label><span><%= p.getProId() %></span></div>
      <div class="field"><label>商品名:</label>
        <input type="text" name="proName" value="<%= p.getProName() %>" required />
      </div>
      <div class="field"><label>単価:</label>
        <input type="text" name="proUnit" value="<%= p.getProUnit() %>"/>
      </div>
      <div class="field"><label>在庫数:</label>
        <input type="text" name="proUnitNum" value="<%= p.getProUnitNum() %>"/>
      </div>
      ...
      <div class="buttons">
        ...
        <button type="button" onclick="location.href='<%=request.getContextPath()%>/ProductManagementServlet';">キャンセル</button>
      </div>
    </form>
  </div>
</body>
