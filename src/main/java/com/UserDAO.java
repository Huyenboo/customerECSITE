...
public class UserDAO extends DBAccess {
	...
	public ArrayList<User> userList() {
		...
		try {
			...
			while (rs.next()) {
				...
				c.setPresidentPhoneNum(rs.getString("president_phone"));
				c.setManagerName(rs.getString("manager_phone_num"));
	...
	public boolean registerUser(User user) {
		String sql = "INSERT INTO app_user (company_name, company_address, president_phone_num, manager_name, manager_phone_num, manager_email, pass, status, requested_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, now())";			
		...
		try {
			...
			ps.setString(8, "pending"); // mã hóa mật khẩu
